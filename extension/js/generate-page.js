        ...
        if (ct.indexOf('json') > -1) {
          ...
          for (let i = 0; i < prodList.items.length; i++) {
            ...
            $.ajax({
              ...
              //contentType: "application/json",
              data: {
                itemId: parseInt(prodList.items[i].id)
              },
              success: function(response, status, xhr) {
                var ct = xhr.getResponseHeader("content-type") || "";
                if (ct.indexOf('json') > -1) {
...
function addCard(start, end, page) {
  for (let i = start; i <= end; i++) {
    ...
    card += '         <!-- Modal for detail -->' +
      ...
      '                            <div class="card-group" name="similar_items" id="card-group-similar' + i + '-1">' +
      '                              <div class="card">' +
...
function addItem(i, price, url, imgUrl, prodTitle, priceThreshold, prodId, timeThreshold, sampleFreq, lowestPrice, historyDates) {
  ...
  // only replace the price when there's a price returned
  if (price){
	$(currentPrice).text(price);
  }
  if (isFinite(min(lowestPrice))){
